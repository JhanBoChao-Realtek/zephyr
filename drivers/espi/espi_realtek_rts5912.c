...
#define CONFIG_ESPI_XEC_PERIPHERAL_HOST_CMD_PARAM_SIZE 256
#define CONFIG_ESPI_XEC_PERIPHERAL_ACPI_SHD_MEM_SIZE   256
#ifdef CONFIG_ESPI_PERIPHERAL_EC_HOST_CMD
static uint8_t ec_host_cmd_sram[CONFIG_ESPI_XEC_PERIPHERAL_HOST_CMD_PARAM_SIZE] __aligned(256);
#endif
#ifdef CONFIG_ESPI_PERIPHERAL_ACPI_SHM_REGION
static uint8_t acpi_shd_mem_sram[CONFIG_ESPI_XEC_PERIPHERAL_ACPI_SHD_MEM_SIZE] __aligned(256);
#endif
...
static void espi_rst_isr(const struct device *dev)
{
	...
	struct espi_rts5912_data *data = (struct espi_rts5912_data *)(dev->data);
...
{
	const struct espi_rts5912_config *const espi_config = dev->config;
	struct espi_rts5912_data *espi_data = (struct espi_rts5912_data *)(dev->data);
	ESPI_Type *espi_reg = (ESPI_Type *)espi_config->espislv_base;
...
static void espi_port81_isr(const struct device *dev)
{
	const struct espi_rts5912_config *const espi_config = dev->config;
	struct espi_rts5912_data *espi_data = (struct espi_rts5912_data *)(dev->data);
	...
	PORT80_Type *port81_reg = (PORT80_Type *)espi_config->port81_base;

	evt.evt_data = port81_reg->DATA;
	espi_send_callbacks(&espi_data->callbacks, dev, evt);
}
#endif
...
static void promt0_ibf_isr(const struct device *dev)
{
	...
	LOG_DBG("APEC: CALLBACK\n");
	LOG_DBG("APEC: STS = 0x%x, 0x%x, 0x%x\n", promt0_reg->STS, ec_host_cmd_sram[2],
		ec_host_cmd_sram[3]);
	espi_send_callbacks(&data->callbacks, dev, evt);
}
...
static int espi_promt0_setup(const struct device *dev)
{
	...
	if (promt0_reg->STS & ACPI_STS_IBF_Msk) {
		rc = promt0_reg->IB;
		LOG_DBG("APEC: rc = 0x%x\n", rc);
	}

	if (promt0_reg->STS & ACPI_STS_IBF_Msk) {
		LOG_ERR("APEC: force clear 0x%x\n", promt0_reg->STS);
		promt0_reg->IB_b.IBCLR = 1;
	}

	LOG_DBG("APEC: promt0_reg->STS = 0x%x\n", promt0_reg->STS);

	promt0_reg->PTADDR = (0x200 | (0x04 << ACPI_PTADDR_OFFSET_Pos));
...
static int VW_Set_Index(uint8_t Index, uint8_t Data,
			const struct espi_rts5912_config *const espi_config)
{
	...
	espi_reg->EVTXDAT = i;
	while (espi_reg->EVSTS_b.TXFULL == 1 && --retry_counter) {
	}

	espi_reg->EVSTS |= BIT(16); /* SetVWTransclr */
...
static int IRQ1_81_Flag;

static void VW_Set_Index_with_KBCData(uint8_t Index, uint8_t Data, uint32_t KBCData,
				      const struct espi_rts5912_config *const espi_config)
{
	uint32_t i;
	uint32_t timeout_cnt = 0;
			 ...
			 const struct espi_rts5912_config *const espi_config)
{
	...
	if (IRQ1_81_Flag == 1) {
		VW_Set_Index(0x0, 0x01, espi_config);
	}

	if (kbc_int_en) {
		IRQ1_81_Flag = 1;
		VW_Set_Index_with_KBCData(0x0, 0x81, exData, espi_config);
	} else {
		kbc_reg->OB = exData;
	}
					  ...
					  uint32_t *data)
{
	...
	if (op >= E8042_START_OPCODE && op <= E8042_MAX_OPCODE) {
		...
		case EACPI_WRITE_CHAR:
			LOG_INF("ECAPI WRITE CHAR OP: 0x%02x, OB=0x%02x, STS=0x%02x",
				(*data & 0xff), acpi_reg->OB, acpi_reg->STS);
		...
		case EACPI_WRITE_STS:
			...
			LOG_INF("ECAPI WRITE STS OP: 0x%02x, OB=0x%02x, STS=0x%02x", (*data & 0xff),
				acpi_reg->OB, acpi_reg->STS);
			break;
		default:
			return -EINVAL;
		}
		...
		case ECUSTOM_HOST_CMD_SEND_RESULT:
			LOG_DBG("APEC: return 0x200 %d", (*data & 0xff));
			promt0_reg->STS &= ~ACPI_STS_STS0_Msk;
...
static int espi_peri_ch_port81_setup(const struct device *dev)
{
	const struct espi_rts5912_config *const espi_config = dev->config;
	struct rts5912_sccon_subsys sccon;
	...
	port81_reg->ADDR = 0x81ul;
	port81_reg->CFG = (PORT80_CFG_CLRFLG_Msk | PORT80_CFG_THREEN_Msk);
	port81_reg->INTEN = PORT80_INTEN_THREINTEN_Msk;

	IRQ_CONNECT(DT_IRQ_BY_NAME(DT_NODELABEL(espi0), port81, irq),
		    DT_IRQ_BY_NAME(DT_NODELABEL(espi0), port81, priority), espi_port81_isr,
		    DEVICE_DT_GET(DT_NODELABEL(espi0)), 0);
	irq_enable(DT_IRQ_BY_NAME(DT_NODELABEL(espi0), port81, irq));
	return 0;
}
#endif
