...
static void kbc_obe_isr(const struct device *dev)
{
	...
	if (espi_data->kbc_pre_irq1 == 1) {
		if (!espi_send_vw_event(0x0, 0x01, dev)) {
				   ...
				   uint8_t level)
{
	...
	while (espi_reg->EVSTS & ESPI_EVSTS_TXFULL && rd_cnt--) {
		k_busy_wait(10);
	}
...
#define ESPI_VW_EVENT_RETRY_CNT   1024
#define ESPI_VW_EVENT_TIMEOUT_CNT 10000

static int espi_send_vw_event(uint8_t index, uint8_t data, const struct device *dev)
{
	const struct espi_rts5912_config *const espi_config = dev->config;
	uint32_t i;
	uint16_t retry_counter = ESPI_VW_EVENT_RETRY_CNT;
...
static int espi_flash_ch_setup(const struct device *dev)
{
	...
	espi_data->maf_ptr = k_aligned_alloc(4, MAF_BUFFER_SIZE);
...
void espi_vw_ch_setup(const struct device *dev);

#define VW_RESET_DELAY 150
